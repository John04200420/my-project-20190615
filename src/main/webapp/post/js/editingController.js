...
function permissionGranted(){
	//取消提示文字，開啟錄音按鈕
	for(var i=0; i<permissionText.length; i++){
...
function recordStartClick(){
	...
	var scriptNo = parseInt(this.parentNode.getElementsByClassName('scriptNo')[0].innerText);
	var scriptPosition = parseInt(this.parentNode.getElementsByClassName('scriptPosition')[0].innerText);
	...
	rec.record();
	if (audioPlayer[index].childNodes.length == 0){
		counter++;
		console.log(counter);
	}
	...
	recordTimeout = setTimeout(function(){
		...
		rec.exportWAV(function(blob){
			...
			playIntervalRecordButton.innerHTML="撥錄音";
...
function playIntervalRecordClick(startTime, duration){
	...
	for(i=0; i<script.length; i++){
		if(audioPlayer[i].childNodes.length>0){
